...
const ghFetch = (path: string, token: string, options: RequestInit = {}) =>
  fetch(path.startsWith("http") ? path : `${GITHUB_API}${path}`, {
    ...options,
    headers: {
...
export const repoExists = async (owner: string, repo: string, token: string) =>
  ghFetch(`/repos/${owner}/${repo}`, token)
    .then((res) => res.ok)
    .catch(() => false);

export const ensureRepoExists = async (config: GitHubConfig) => {
...
const getCommitTreeSha = async (commitSha: string, config: GitHubConfig) => {
  const { owner, repo, token } = config;
...
export const listCommitsForPath = async (
  config: GitHubConfig,
  path: string,
  maxCount = 500,
...
const createInitialCommitViaContents = async (
  filePath: string,
  content: string,
  message: string,
  config: GitHubConfig,
...
export const pushCommitHistory = async (
  commits: CommitInput[],
  config: GitHubConfig,
  ...
  if (parentSha === null && commits.length > 0) {
    ...
    console.log(
      `[${new Date().toISOString()}] sync: empty repo - bootstrapping via Contents API`,
    );
  ...
  for (let i = 0; i < commits.length; i++) {
    ...
    if ((i + 1) % 50 === 0 || i === 0) {
      console.log(
        `[${new Date().toISOString()}] sync: pushed ${done}/${totalCommits} commits`,
      );
    }

    const blobSha = await createBlob(c.content, config);
    const baseTree = parentSha
      ? await getCommitTreeSha(parentSha, config)
      : undefined;
